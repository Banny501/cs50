...
void tabulate(void)
{
    for (int v = 0; v < voter_count; v++)
    {
        ...
        {
            if (!candidates[preferences[v][c]].eliminated)
            {
...
int find_min(void)
{
    ...
    for (int a = 0; a < candidate_count; a++)
    {
        if (candidates[a].votes <= vote_min && candidates[a].eliminated == false)
        {
            vote_min = candidates[a].votes;
        }
    }
